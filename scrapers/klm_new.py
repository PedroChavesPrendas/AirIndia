...
def login(username, password):
    ...
    if req.history:
        ...
        return None
def get_klm_data(username,password, retry=0):
    """
    ...
    if not session:
        if retry < 3:
            return get_klm_data(username, password, retry=retry+1)
        else:
            return False, 0
    ...
    try:
        ...
        for row in body:
            ...
            with tempfile.NamedTemporaryFile(delete=False) as f:
                f.write(req.content)
                status, s3_link = upload_s3(f.name, row_text[6], 'klm')
                if status:
                    ...
                    try:
                        ...
                        logger.exception(f"Error inserting data into PostgreSQL:{e}")
                    finally:
                        message={
...
#     username = "AADCI3006N"
#     password = "AADCI3006N"
#     get_klm_data(username, password)
# main()

def main(username):
    # username = "AADCI3006N"
    # password = "AADCI3006N"
    get_klm_data(username, username)

def read_credentials_and_run():
    with open('klm_pass.csv', 'r') as csvfile:
